...
const postRoutes = (app) => {
    ...
    app.post('/api/posts/create/', async (req, res) => {
        ...
        try {
            const data = req.body
            const { title, userName, content, country, email } = data

            if (!title || !userName || !content || !country || !email) return res.status(401).json({ message: 'title, userName, content are required!' })
